...
let animation = lottie.loadAnimation({
    ...
    loop: true,
    autoplay: true,
    path: "https://lottie.host/caa3e842-7f52-44f2-a825-82ebb0e6667a/q5re4mGr6s.json" // رابط الأنيميشن الأول
});
...
function startFinalMessageRotation() {
    clearInterval(messageInterval); // تأكد من إيقاف الرسائل المتكررة
    finalMessageIndex = 0; // إعادة تعيين المؤشر
    document.getElementById("loading-message").textContent = finalMessages[finalMessageIndex]; // عرض الرسالة الأولى فورًا
    setInterval(() => {
...
animation = lottie.loadAnimation({
    ...
    clearInterval(messageInterval); // إيقاف الرسائل القديمة
    startFinalMessageRotation(); // بدء الرسائل النهائية
...
startButton.addEventListener("click", function () {
    ...
    animation.destroy();
    animationContainer.style.width = "250px";  // قم بتغيير الحجم كما تريد
    animationContainer.style.height = "250px"; // قم بتغيير الحجم كما تريد
